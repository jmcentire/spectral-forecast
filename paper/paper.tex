...
We evaluate on the four ETT (Electricity Transformer Temperature) datasets introduced by Zhou et al.~\cite{zhou2021}: ETTh1, ETTh2 (hourly, $17{,}420$ points each), ETTm1, ETTm2 ($15$-minute intervals, $69{,}680$ points each). We forecast the OT (oil temperature) column, following the standard univariate protocol.
...
\begin{table}[t]
\caption{Normalized MSE on ETT benchmarks. Bold indicates best. TimesFM and PatchTST numbers at $H=96$ from published results; longer horizons shown as ``---'' where published comparisons are unavailable at that exact configuration.}
\label{tab:main}
\centering
\small
...
On ETTh2 and ETTm2, the transformer baselines win. TimesFM achieves $0.289$ on ETTh2 versus our $0.400$, and $0.175$ on ETTm2 versus our $0.251$. The method's advantages and disadvantages are dataset-dependent, not universal.
...
\textbf{Shock absorption.} When a large discrete shock (step function) occurs in the middle of the context, its broadband spectral energy is partially absorbed by Layer~1 before Layer~3a can detect it. A pre-pass shock detector would fix this but adds complexity. The current architecture handles shocks near the end of the context well.
...
\textbf{Limited benchmark scope.} We evaluate on four ETT datasets. A full comparison would include Weather, Traffic, Electricity, and the Monash archive. The method's performance on these datasets remains to be established.
